...
export default function LabCoordinatorLabsPage() {
  ...
  const loadComponents = async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/lab-coordinator/labs/components', { cache: 'no-store' })
      if (res.ok) {
  ...
  const generateComponentLogPDF = async (log: ComponentLog, viewOnly: boolean = false) => {
    ...
    // Show final approver - either Lab Coordinator or HOD
    if (log.lab_coordinator_name) {
      yPos = checkAddPage(yPos, 20)
      doc.setFont('helvetica', 'bold')
      doc.text('✓', 20, yPos)
      ...
      // If no Lab Coordinator approval, show HOD approval
      yPos = checkAddPage(yPos, 20)
      doc.setFont('helvetica', 'bold')
      doc.text('✓', 20, yPos)
      doc.setFont('helvetica', 'normal')
