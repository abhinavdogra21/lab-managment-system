...
interface RequestItem {
  ...
  lab_coordinator_remarks?: string
    ...
    <div className="space-y-3">
      <div className="px-2">
        ...
        <div className="mt-3 space-y-2">
          ...
          {item.lab_coordinator_remarks && (
            <div className="text-xs p-2 bg-gray-50 rounded border-l-2 border-blue-300">
              <span className="font-medium">Lab Coordinator:</span> {item.lab_coordinator_remarks}
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <Card className="hover:shadow-md transition-shadow">
      <CardContent className="p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-sm">
              {item.is_multi_lab ? `Multi-Lab Booking (${item.multi_lab_approvals?.length || 0} labs)` : item.lab_name}
            </span>
              ...
              <div key={approval.lab_id} className="bg-white p-2 rounded space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <Badge 
                    ...
                    className="text-xs"
                  >
                    {approval.status === 'approved' ? 'Approved by Lab Coordinator' : 
                     approval.status === 'approved_by_lab_staff' ? 'Pending Lab Coordinator' :
              ...
              <div className="flex items-center justify-between">
                ...
                <Badge 
                  variant={
                    ...
                    item.status === 'pending_hod' || item.status === 'pending_lab_coordinator' || item.lab_staff_approved_at ? 'default' :
                    item.status === 'pending_lab_staff' ? 'secondary' : 
                    'destructive'
                  }
                  className="text-xs"
                >
                  {item.status === 'approved' ? 'Approved by Lab Coordinator' : 
                   item.status === 'pending_hod' || item.status === 'pending_lab_coordinator' || item.lab_staff_approved_at ? 'Pending Lab Coordinator' :
        ...
        {(item.faculty_remarks || item.lab_staff_remarks || item.lab_coordinator_remarks) && (
          <div className="space-y-1">
            ...
            {item.lab_coordinator_remarks && (
              <div className="bg-purple-50 p-2 rounded text-xs">
                <span className="font-medium text-purple-800">Lab Coordinator: </span>
                <span className="text-purple-600 italic">"{item.lab_coordinator_remarks}"</span>
              </div>
            )}
          </div>
        )}
...
export default function LabCoordinatorApprovePage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending_hod':
        return <Badge variant="outline" className="text-purple-600 border-purple-300">Pending Lab Coordinator Review</Badge>
      case 'approved':
