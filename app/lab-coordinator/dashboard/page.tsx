...
export default function LabCoordinatorDashboardPage() {
  ...
  const loadDashboardStats = async () => {
    ...
    try {
      ...
      const labsRes = await fetch('/api/lab-coordinator/labs/components', { cache: 'no-store' })
      if (labsRes.ok) {
      ...
      const pendingRes = await fetch('/api/lab-coordinator/requests?status=pending_hod', { cache: 'no-store' })
      if (pendingRes.ok) {
      ...
      const approvedRes = await fetch('/api/lab-coordinator/requests?status=approved', { cache: 'no-store' })
      if (approvedRes.ok) {
      ...
      const rejectedRes = await fetch('/api/lab-coordinator/requests?status=rejected', { cache: 'no-store' })
      if (rejectedRes.ok) {
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="space-y-1">
        <h1 className="text-3xl font-bold">Lab Coordinator Dashboard</h1>
        <p className="text-muted-foreground">
          Manage your department labs and track activities
        </p>
      </div>
