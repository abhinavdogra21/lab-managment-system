...
export default function OrganizationPage() {
  ...
  const removeFacultyFromDept = async (userId: number) => {
    setLoading(true)
    try {
  const res = await fetch(`/api/admin/users/${userId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ department: null }) })
      const data = await res.json()
  ...
  const setFacultyDepartment = async (userId: number, deptCode: string | null) => {
    setLoading(true)
    try {
  const res = await fetch(`/api/admin/users/${userId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ department: deptCode }) })
      const data = await res.json()
  ...
  const addExistingFaculty = async () => {
    ...
    setLoading(true)
    try {
  const res = await fetch(`/api/admin/users/${fid}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ department: facultyDept.code }) })
      const data = await res.json()
                                    ...
                                    <input
                                      ...
                                      onChange={async (e) => {
                                        ...
                                        try {
                                          ...
                                          const res = await fetch(`/api/admin/labs`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ labId: l.id, staffIds: Array.from(next) }) })
                                          const data = await res.json()
                                          if (!res.ok) throw new Error(data?.error || "Failed to update lab")
                                          const staffList: Array<{ id: number; name: string }> = data.staff || []
                                ...
                                <AlertDialogFooter>
                                  ...
                                  <AlertDialogAction onClick={async () => {
                                    ...
                                    try {
                                      ...
                                      if (currentLab) {
                                        const res1 = await fetch(`/api/admin/labs`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ labId: currentLab.id, staffId: null }) })
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button disabled={loading} onClick={async () => {
              ...
              setLoading(true)
              try {
                const res = await fetch("/api/admin/users", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: fullName, email: staffForm.email, role: "lab_staff" }) })
                const data = await res.json()
                if (!res.ok) throw new Error(data?.error || "Failed to add staff")
                setLabStaff((prev) => [...prev, data.user])
                if (staffForm.labId) {
                  const res2 = await fetch(`/api/admin/labs`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ labId: staffForm.labId, staffId: data.user.id }) })
