...
export default function FacultyBookLabsPage() {
	...
	const loadAvailable = async (labId: string, date: Date) => {
		try {
			...
			const res = await fetch(`/api/faculty/available-slots?lab_id=${labId}&date=${dateStr}`)
			if (res.ok) {
				const data = await res.json()
	...
	const loadBooked = async (labId: string, date: Date) => {
		try {
			const dateStr = format(date, 'yyyy-MM-dd')
			const res = await fetch(`/api/faculty/booked-slots?lab_id=${labId}&date=${dateStr}`)
			if (res.ok) {
				const data = await res.json()
	...
	const handleSubmit = async () => {
		...
		try {
			...
			const res = await fetch('/api/faculty/booking-requests', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
			const data = await res.json()
		...
		<div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
			<div className="flex items-center gap-4 mb-6">
				...
				<div>
					...
					<p className="text-muted-foreground">Faculty can raise a booking that goes to Lab Staff, then HOD</p>
				</div>
			</div>
							...
							{selectedDate && (
								<div className="space-y-4">
									...
									<div className="p-3 border border-blue-200 bg-blue-50 text-sm text-blue-700 rounded">
										<div className="flex items-center gap-2 mb-1"><Clock className="h-4 w-4" /><strong>Lab Booking Hours</strong></div>
					...
					<Card>
						...
						<CardContent className="space-y-4">
							<div>
								...
								<Textarea placeholder="Describe why you need the lab" value={purpose} onChange={(e) => setPurpose(e.target.value)} rows={4} />
							</div>
							<div className="flex gap-2 pt-4">
