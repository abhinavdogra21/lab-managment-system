...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build WHERE clause for assigned labs
    const labIdsPlaceholder = assignedLabIds.map(() => '?').join(',')
    
    // Get timetable entries for the day (only for assigned labs)
    const timetableQuery = labId
      ? `SELECT 
         ...
         AND te.lab_id IN (${labIdsPlaceholder})
    ...
    const timetableParams = labId 
      ? [dayOfWeek, labId, ...assignedLabIds] 
      : [dayOfWeek, ...assignedLabIds]
    ...
    const bookingsParams = labId 
      ? [date, labId, ...assignedLabIds] 
      : [date, ...assignedLabIds]
