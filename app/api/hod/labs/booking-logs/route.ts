    ...
    if (user.role === 'admin') {
      logsRes = await db.query(
        `SELECT 
         ...
         WHERE br.status = 'approved' AND br.request_type = 'lab_booking'
         ORDER BY br.booking_date DESC, br.start_time DESC
         LIMIT 100`
      )
    } else {
      ...
      logsRes = await db.query(
        `SELECT 
         ...
         LEFT JOIN users lab_staff ON br.lab_staff_approved_by = lab_staff.id
         LEFT JOIN users hod ON d.hod_id = hod.id
         WHERE br.status = 'approved' AND br.request_type = 'lab_booking' AND l.department_id IN (${placeholders})
         ORDER BY br.booking_date DESC, br.start_time DESC
         LIMIT 100`,
        departmentIds
      )
