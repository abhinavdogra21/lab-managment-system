...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Notify faculty if they have already approved (status is NOT pending_faculty)
    if (booking.status !== "pending_faculty" && booking.faculty_email) {
      if (!emailsSent.has(booking.faculty_email)) {
    ...
    if (["pending_lab_staff", "pending_hod", "approved"].includes(booking.status)) {
      if (booking.is_multi_lab) {
        // For multi-lab bookings, notify head lab staff only (from labs.staff_id)
        let labIds: number[] = []
        if (Buffer.isBuffer(booking.lab_ids)) {
          labIds = JSON.parse(booking.lab_ids.toString('utf-8'))
        } else if (typeof booking.lab_ids === 'string') {
        ...
        // For single lab bookings, notify head lab staff only
    ...
    for (const recipient of emailRecipients) {
      ...
      const withdrawalMessage = recipient.isRequester
        ? `You have successfully withdrawn your lab booking request.`
        : `A lab booking request has been <strong>withdrawn</strong> by ${booking.requester_name}.`
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p>This booking request has been withdrawn and no further action is required.</p>
            </div>
            <div class="footer">
    ...
    const userInfo = await getUserInfoForLogging(user.id)
    ...
    for (const labId of labIdsToLog) {
      logLabBookingActivity({
        ...
        actorRole: userInfo?.role || null,
        action: "withdrawn",
        actionDescription: `Booking withdrawn by 'others' role requester`,
    ...
    console.error("Others booking withdrawal error:", error)
