...
export default function LabStaffAnalyticsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const url = params.toString()
        ? `/api/lab-staff/analytics/component-utilization?${params.toString()}`
        : '/api/lab-staff/analytics/component-utilization'
  ...
  // Get labs with no component requests
  ...
  // Filter and sort component stats
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Component usage breakdown by lab. Formula: (Total Quantity Requested / Total Quantity Available) × 100. Shows how much of the available inventory has been requested (can exceed 100% for high demand).
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
              ...
              <Table>
                ...
                <TableBody>
                  {filteredLabStats.map((stat, index) => (
                    <TableRow key={`${stat.lab_id}-${stat.component_id || 'null'}-${index}`}>
          ...
          {/* Show labs with no component requests */}
              ...
              <Table>
                ...
                <TableBody>
                  {filteredComponentStats.map((stat, index) => (
                    <TableRow key={`comp-${stat.component_id || 'null'}-${stat.component_name}-${index}`}>
